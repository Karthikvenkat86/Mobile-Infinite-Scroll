  ...
  MediaFire.prototype.getContent = function(indexer, getInfo) {
    ...
    if(this.currentContentStatus.fire_flag) {
      ...
      if((this.currentContentStatus.file_more_chunks == "yes" || indexer === -1) && that.itemCount.folder_count < 100) {
        ...
        if(chunkCount >= 1) {
          this.currentContentStatus.chunk += nextChunk;
          console.log("current file ---" + this.currentContentStatus.chunk, "last file ---" + this.lastChunkCount);
          if(indexer== -1 && this.currentContentStatus.chunk > 1 && this.currentContentStatus.chunk === this.lastChunkCount-1 && !this.lastchunkObserved) {
            this.currentContentStatus.chunk--;
            this.lastchunkObserved = true;
          }
          
          if(!getInfo) {
            if(this.currentContentStatus.chunk !== 1 || this.lastChunkCount !== 2) {
            ...
            setTimeout(function() {
              //console.log("File Request-------", indexer);
